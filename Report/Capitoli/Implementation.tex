...
\begin{itemize}
    ...
    \item \textbf{Interrupt button} connected in pull-up with a 10kΩ resistor;
...
Additionally, an interrupt callback for the button \\
(\texttt{HAL\_GPIO\_EXTI\_Callback()}) sets a flag when the interrupt button is pressed, allowing asynchronous event handling. The callback also generates a UART message to confirm the button press.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{Immagini/adc-raw-plot.png}
\includegraphics[width=0.7\textwidth]{Immagini/adc-ma-plot.png}
\includegraphics[width=0.7\textwidth]{Immagini/adc-noise-plot.png}
